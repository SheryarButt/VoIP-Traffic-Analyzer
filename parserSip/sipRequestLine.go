 ...
 * DESCRIPTION:
		...
		Package containing functions responible for parsing of Request line information from packets.
*/
...
/*
 * FUNCTION_NAME: parseSipReq
 * ARGS:
 *	@_arg1 ([]byte)	: Packet Data in raw bytes.
 *	@_arg2 (sipReq)	: Pass by reference sipReq struct.
 * DESCRIPTION: Parsing routine for SIP Request line parsing.
 */
	...
	for pos < len(v) {
		...
		case FIELD_HOST:
			if v[pos] == ';' || v[pos] == '>' || v[pos] == ' ' {
				...
				pos++
				continue
			} else if (!ipv6 && v[pos] == ':') || (!ipv6 && v[pos] == ']' && v[pos+1] == ':') {
				state = FIELD_PORT
				pos++
				continue
			}
